...
mapping = { "x" => "a", "y" => "b", "z" => "c" }
mappingToLoose = { "a" => "c", "b" => "a", "c" => "b" }
mappingToWin = { "a" => "b", "b" => "c", "c" => "a" }
...
roundsT.each do |i|
  if i.length > 0
    opponentDraw, reqResult = i.downcase.split( " " )
    ownDraw = if reqResult == "x" # loose
                mappingToLoose[opponentDraw]
              elsif reqResult == "y"
                opponentDraw
              else
                mappingToWin[opponentDraw]
              end
